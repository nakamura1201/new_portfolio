...
description: 事業探索から要件定義までの全工程を統括するオーケストレータースキル。overview定義、事前ユーザーヒアリング、発散、根本原因、ペルソナ分析、カスタマージャーニー分析、事業仮説、市場調査、検証設計、課題検証、MVP定義、MVP学習、KPI計測設計、リスク法務運用チェック、Go/No-Go、PRD作成、リリース計画、定例レビュー運用、変更管理を順番実行し、成果物の整合性を管理する。全体フローを一括で進めたい依頼時に使う。
---
...
2. `discovery-step-0-user-hearing`
3. `discovery-step-1-diverge-structure`
4. `discovery-step-2-root-cause-n1`
5. `discovery-step-persona-analysis`
6. `discovery-step-customer-journey`
7. `discovery-step-3-business-hypothesis`
8. `discovery-step-market-research`
9. `discovery-step-validation-design`
10. `discovery-step-4-problem-fit-validation`
11. `discovery-step-5-mvp-definition`
12. `discovery-step-6-mvp-learning`
13. `discovery-step-kpi-measurement-design`
14. `discovery-step-risk-legal-ops-check`
15. `discovery-step-go-no-go`
16. `discovery-step-7-prd-authoring`
17. `discovery-step-release-planning`
18. `discovery-step-review-ritual`
19. `discovery-step-change-management`
20. `overview-content-enhancer`

## 統括ルール
- `discovery-step-1-diverge-structure` の開始条件は `discovery-step-0-user-hearing` 完了とする。
- 全ステップ完了後に `overview-content-enhancer` で `doc/01_overview` を最終確定する。
- Discovery完了時に全体結果の集約サマリーを `doc/05_marketing/01_product_marketing/final-summary.md` に作成・更新する。
...
- `doc/05_marketing/01_product_marketing/final-summary.md`（Discovery全体の要約・意思決定・次アクション）
- `doc/05_marketing/01_product_marketing/step-00-ユーザーヒアリング/`（ヒアリング設計・ログ・示唆）
